...
function Header() {
    ...
    const onSubmit = (data) => {
        ...
        navigate(`${location.pathname}?query=${encodeURIComponent(data.Search)}`);
    };
    ...
    const navItems = [
        ...
        {
            name: "Videos",
            path: `/results/videos?query=${encodeURIComponent(query)}`
        },
    ]

    return (
        <div className='sticky top-0 max-w-screen dark:bg-[#242424] h-auto m-0 p-2'>
            <div className='flex flex-wrap flex-row justify-between items-center mb-4 '>
                <Link to="/" className='cursor-pointer p-1'>
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <li key={item.name}>
                                <Link
                                    to={item.path}
                                    className={`pb-1 font-semibold ${isActive
                                        ? "text-[#1f1f1f] hover:text-[#1f1f1f] dark:text-[#e8e8e8] dark:hover:text-[#e8e8e8] border-b-2 border-white"
                                        : "text-[#80868b] hover:text-[#1f1f1f] dark:hover:text-[#dbdbdb]"
                                        }`}
                                >
                                    {item.name}
                                </Link>
            ...
            <hr className="border-0 h-[1.5px] dark:bg-[#313335] bg-[#dadce0] my-2" />
        </div>
    )
}
